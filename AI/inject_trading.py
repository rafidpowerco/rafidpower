...
async def ingest_trading():
    files = [f for f in os.listdir(r'C:\Users\Administrator\Desktop\ai_training_questions') if f.startswith('trading_analysis') and f.endswith('.json')]
    print(f'Found {len(files)} new trading files.')
    for f in files:
        ...
        for idx, item in enumerate(data):
            ...
            if ans:
                collection.add(
                    ...
                    metadatas=[{'category': 'Trading_Analysis', 'source': 'External_Data'}],
                    ids=[f'trading_{int(time.time())}_{f}_{idx}']
                )
            await asyncio.sleep(2)
        print(f'{f} injected successfully.')

asyncio.run(ingest_trading())
